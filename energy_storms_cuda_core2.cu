...
void particles_bombardment_kernel(int *d_particles, float *d_layer, int layer_sz){
    // Each thread represents a particle
    int tid = threadIdx.x + blockIdx.x * blockDim.x 

    float energy = d_particles[tid*2+1] * 1000.0f;
    int position = d_particles[tid*2];

    for(int k = 0; k < layer_sz; k++) {
        updateAtomic(d_layer, layer_sz, k, position, energy );
    }
}

void particles_bombardment_kernel_soa(float *d_energies, int *d_positions, float *d_layer, int layer_sz){
    // Each thread represents a particle
    int tid = threadIdx.x + blockIdx.x * blockDim.x 

    float energy = energies[tid] * 1000.0f;
    int position = positions[tid];
